...
getUserName = function(user){
  return user.username || getGoogleUserName(user) || getProperty(user, 'services.twitter.screenName');
}
...
getGoogleEmail = function (user) {
  return getProperty(user, 'services.google.email');
}

getGoogleUserName = function (user){
  googleEmail = getGoogleEmail(user);
  googleEmailAndDomainArray = googleEmail.split("@");
  return googleEmailAndDomainArray[0];
}

getEmailDomain = function (user) {
  email = getEmail(user);
  emailAndDomainArray = email.split("@");
  return emailAndDomainArray[1];
}
